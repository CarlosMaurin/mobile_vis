...
// Prop extra solo para el callback interno — no forma parte del tipo público
interface TestimonialCardInternalProps extends TestimonialProps {
  onLogoLoad?: () => void;
}

const TestimonialCard: React.FC<TestimonialCardInternalProps> = ({ logo, company, quote, onLogoLoad }) => {
...
const Testimonials: React.FC = () => {
  ...
  // Contador de logos cargados — cuando llega al total, recalcula bounds
  ...
  // Recalcula cuando todos los logos terminaron de cargar
  // Esto resuelve el bug de primera visita: los logos de Cloudinary tienen
  // aspect ratios variables y hasta que no cargan el layout de las cards
  // está incompleto, por lo que calculateBounds medía mal.
  // En visitas posteriores las imágenes están cacheadas y no había problema.
  useEffect(() => {
    if (logosLoaded < TOTAL_LOGOS) return;
    if (!trackRef.current) return;
  ...
  // Fase 3: Scroll Horizontal — mecanismo original sin modificar
        ...
        <motion.div style={{ y: cardsY, opacity: cardsOpacity }} className="relative z-10 w-full">
          ...
          >
            {testimonials.map((t, i) => (
              <TestimonialCard key={i} {...t} onLogoLoad={handleLogoLoad} />
            ))}
