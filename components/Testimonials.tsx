...
const TestimonialCard: React.FC<TestimonialProps> = ({ logo, company, quote }) => {
  ...
  return (
    ...
    >
      {/*
        Contenedor de dimensiones FIJAS para el logo.
        Ancho fijo + alto fijo = el layout de la card es idéntico
        antes y después de que cargue la imagen. Así calculateBounds
        siempre mide lo mismo, igual que hacía el avatar w-12 h-12.
        El logo se escala dentro sin deformar (object-contain).
      */}
      <div
        className="flex justify-center items-center mb-[clamp(14px,1.8vw,22px)]"
        style={{ width: '100%', height: 'clamp(44px,5vw,64px)' }}
      >
        <img
          src={logo}
          alt={company}
          style={{ maxHeight: '100%', maxWidth: '70%', objectFit: 'contain', display: 'block' }}
        />
      </div>
